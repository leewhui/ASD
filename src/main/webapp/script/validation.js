...
function checkPasswordMatch()
{
    ...
    {
        document.getElementById('passwordText').innerHTML = "Error. At least 8 - 16 characters, with one capital letter, one lower case and one number";
        return false;
    }
}
...
emailBox.onblur = function() {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (msg === "true")
            {
                document.getElementById('emailText').innerHTML = 'This email address is already exist';
    ...
    xhr.open('post', 'checkEmailUnique', true   );
    xhr.setRequestHeader("content-type","application/x-www-form-urlencoded");
    xhr.send("email="+emailBox.value);
}
...
nextBtn.onclick = function()
{
    ...
    if(confirmBox.value === '') document.getElementById('confirmText').innerHTML = 'Please confirm your password';
